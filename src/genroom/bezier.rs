...
impl RoomGen for Bezier {
	fn generate(
		...
		pts.push(match b4.sample(rng) {
			0 => [xrange.sample(rng), y],
			1 => [xrange.sample(rng), y + h - 1.0],
			2 => [x, yrange.sample(rng)],
			3 => [x + w - 1.0, yrange.sample(rng)],
			_ => unreachable!(),
		});
		for _ in 0..self.pnum {
			pts.push([xrange.sample(rng), yrange.sample(rng)]);
		}
		pts.push(match b4.sample(rng) {
			0 => [xrange.sample(rng), y],
			1 => [xrange.sample(rng), y + h - 1.0],
