...
use tick::Ticker;
use util::{Char, Curse, EXITGAME, R};
...
fn main() {
	...
	fn rateaterfn(rateater: Entity, w: &mut World) {
		let mut seeking = w.write_storage::<Seeking>();
		seeking.insert(
			rateater,
			Seeking(
				Seek::Race(Race::Raffbarf),
				Action::Action(Box::new(move |w| {
	...
	let mut rng = R::from_rng(&mut rand::thread_rng()).expect("Failed to seed rng");
	{
		...
		for fxy in f1.iter() {
			rng.choose(&genbag).unwrap().generate(
				&mut rng,
				[fxy[0], fxy[1], 1],
				fxy[2] - fxy[0] + 1,
				fxy[3] - fxy[1] + 1,
