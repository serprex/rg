...
use util::{rectover, rectoverinc, R};
...
pub fn init(rng: &mut R, n: usize, minwh: i16, dim: [i16; 4], inc: bool) -> Vec<[i16; 4]> {
	...
	let n = cmp::min(
		n,
		(w as usize * h as usize) / (minwh as usize * minwh as usize),
	);
	if n < 2 {
		vec![dim]
	} else {
...
pub fn grow(rng: &mut R, rxy: &mut [[i16; 4]], dim: [i16; 4], inc: bool) -> AdjacencySet {
...
pub fn joinlist<A: Adjacency>(rng: &mut R, adj: &A, rc: usize) -> Vec<(usize, usize)> {
...
pub fn doors<D>(rng: &mut R, connset: D, rxy: &[[i16; 4]]) -> (FnvHashSet<[i16; 2]>, usize)
where
	D: Iterator<Item = (usize, usize)>,
{
	let mut lastaidx = 0;
