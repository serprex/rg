			...
			if let Some(pos) = possy.get_pos(ent) {
				match *state {
					AiState::PlayerInventory(invp) => {
						...
						let mut armors = w.write_storage::<Armor>();
						if let Some(&mut Bag(ref mut ebag)) = bag.get_mut(ent) {
							'invput: loop {
								match (getch(), ebag.is_empty()) {
									('i', _) => *state = AiState::Player,
									('j', false) => {
										*state = AiState::PlayerInventory(if invp == ebag.len() - 1
										{
											0
										} else {
											invp + 1
									...
									('k', false) => {
										*state = AiState::PlayerInventory(if invp == 0 {
											ebag.len() - 1
										} else {
											invp - 1
										})
									...
									('w', false) => {
										if let Ok(Some(Weapon(oldw))) =
											weapons.insert(ent, Weapon(ebag.remove(invp)))
										{
											ebag.push(oldw);
									...
									('s', false) => {
										if let Ok(Some(Shield(oldw))) =
											shields.insert(ent, Shield(ebag.remove(invp)))
										{
											ebag.push(oldw);
										} else if invp == ebag.len() {
											*state = AiState::PlayerInventory(0);
									...
									('a', false) => {
										if let Ok(Some(Armor(oldw))) =
											armors.insert(ent, Armor(ebag.remove(invp)))
										{
											ebag.push(oldw);
										} else if invp == ebag.len() {
											*state = AiState::PlayerInventory(0);
									...
									('u', false) => {
										let mut consume = w.write_storage::<Consume>();
					...
					AiState::Player => 'playerinput: loop {
						match char_as_dir(getch()) {
							...
							Err('a') => match char_as_dir(getch()) {
								Ok(d) => donow.push(Action::Attack { dir: d, src: ent }),
								_ => continue 'playerinput,
							},
							Err('q') => match char_as_dir(getch()) {
								Ok(d) => donow.push(Action::Lunge { dir: d, src: ent }),
								_ => continue 'playerinput,
							},
							Err('s') => match char_as_dir(getch()) {
					...
					AiState::Scared(foe) => match possy.get_pos(foe) {
						...
						Some(fxy) => {
							...
							for &dir in &[Dir::L, Dir::H, Dir::J, Dir::K] {
								let choice = xyz_plus_dir(pos, dir);
								if (pos[0] - fxy[0]).abs() + (pos[1] - fxy[1]).abs() > dist
									&& !possy.contains(choice)
								{
									choices[chs] = dir;
									chs += 1;
								}
					...
					AiState::Aggro(foe) => match possy.get_pos(foe) {
						None => *state = AiState::Random,
						Some(fxy) => {
							let crace = w.read_storage::<Race>();
							match crace.get(ent) {
								Some(&Race::Leylapan) => {
									...
									let mut dnum = 0;
									if pos[0] != fxy[0] {
										dirs[0] = if pos[0] < fxy[0] { Dir::L } else { Dir::H };
										dnum = 1
									}
									if pos[1] != fxy[1] {
										dirs[dnum] = if pos[1] < fxy[1] { Dir::J } else { Dir::K };
										dnum += 1
									}
									if let Some(&fdir) = rng.choose(&dirs[..dnum]) {
										let mut weight = w.write_storage::<Weight>();
										let mut fragile = w.write_storage::<Fragile>();
										let mut cch = w.write_storage::<Chr>();
								...
								_ => {
									...
									if !attacking {
										...
										loop {
											let mut xy = xxyy;
											let co = if tries == 1 || (tries == 3 && rng.gen()) {
												0
											} else {
												1
											};
										...
										if xxyy == fxy {
											let co =
												if pos[0] != fxy[0] && rng.gen() { 0 } else { 1 };
											if let Some(dir) =
												match (co, cmpi(pos[co], fxy[co], 1, 0, -1)) {
													(0, -1) => Some(Dir::H),
													(0, 1) => Some(Dir::L),
													(1, -1) => Some(Dir::K),
													(1, 1) => Some(Dir::J),
					...
					AiState::Ally(_ally, ref astate) => match *astate {
						AllyState::Random => (),
						AllyState::Follow => (),
						AllyState::Aggro(_) => (),
						AllyState::Scared(_) => (),
